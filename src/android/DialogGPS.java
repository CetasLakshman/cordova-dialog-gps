...
public class DialogGPS extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (this.cordova.getActivity().isFinishing())
            ...
            if (!gpsEnable) {
                this.createDialog(args.getString(0), args.getString(1), args.getString(2), args.getJSONArray(3),
                        callbackContext);
                return true;
            }
    ...
    /**
     * Construct de GPS dialog with de title,message and buttons especified. When
     * positive button is pressed then its redirection to enable GPS
     *
     ...
     * @param buttonLabels{JSONArray} array of labels to the positive and negative
     *        button.
     * @param callbackContext{CallbackContext} callback to display the result.
     **/
    public synchronized void createDialog(final String title, final String message, final String description,
            ...
            public void run() {
                ...
                if (buttonLabels.length() == 2) {
                    ...
                    try {
                        ...
                        builder.setNeutralButton(buttonLabels.getString(1), new AlertDialog.OnClickListener() {
                ...
                if (buttonLabels.length() == 3 || buttonLabels.length() == 2) {
                    try {
                        builder.setNegativeButton(buttonLabels.getString(0), new AlertDialog.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                dialog.dismiss();
                                isShown = false;
                        ...
                        builder.setPositiveButton(positiveButtonName, new AlertDialog.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                dialog.dismiss();
                                isShown = false;
     ...
     *
     * @param cordova CordovaInterface to append the dialog created. the dialog is
     *                forced to display with the theme light default
     **/
    @SuppressLint("NewApi")
    private AlertDialog.Builder newDialog(CordovaInterface cordova, final String title, final String message,
        ...
        LinearLayout.LayoutParams _layout_params = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
        ...
        _icon.setImageResource(
                cordova.getActivity().getResources().getIdentifier("ic_location", "drawable", packageName));
        ...
        _description.setLayoutParams(
                new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, dpToPixels(72, dpi)));
